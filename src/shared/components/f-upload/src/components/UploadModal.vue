<template>
  <BasicModal
    width="800px"
    :title="t('Unified.texts.component:upload:upload')"
    :okText="t('Unified.texts.component:upload:save')"
    v-bind="$attrs"
    @register="register"
    @ok="handleOk"
    ...
    <div class="upload-modal-toolbar">
      ...
      >
        <a-button type="primary">
          {{ t('Unified.texts.component:upload:choose') }}
        </a-button>
      </Upload>
    </div>
    <FileList
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['change', 'register', 'delete', 'beforeUpload']);
  ...
  const getUploadBtnText = computed(() => {
    ...
    return isUploadingRef.value
      ? t('Unified.texts.component:upload:uploading')
      : someError
        ? t('Unified.texts.component:upload:reUploadFailed')
        : t('Unified.texts.component:upload:startUpload');
  });
  ...
  async function handleStartUpload() {
    ...
    if (fileListRef.value.length + props.previewFileList.length > maxNumber) {
      return createMessage.warning(t('Unified.texts.component:upload:maxNumber', [maxNumber]));
    }
    try {
  ...
  function handleOk() {
    ...
    if (isUploadingRef.value) {
      return createMessage.warning(t('Unified.texts.component:upload:saveWarn'));
    }
    ...
    if (fileList.length <= 0) {
      return createMessage.warning(t('Unified.texts.component:upload:saveError'));
    }
    fileListRef.value = [];
    closeModal();
  ...
  async function handleCloseFunc() {
    if (!isUploadingRef.value) {
      ...
      createMessage.warning(t('Unified.texts.component:upload:uploadWait'));
      return false;
    }
  }
</script>
