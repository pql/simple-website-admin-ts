import { CardLabelService } from '@/apis';
...
import CreateoOrEditCardLabel from './create-or-edit-card-label/create-or-edit-card-label.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      queryFilter.filterName = queryFilter.filterCardLabel;
      return CardLabelService.getApiAppCardLabelPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewCardLabel' : 'Unified.texts.EditCardLabel',
      };
      this.modalHelper
        .create(CreateoOrEditCardLabel, { ...param }, { width: 500 })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      CardLabelService.deleteApiAppCardLabel({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditCardLabel',
      };
      this.modalHelper
        .create(CreateoOrEditCardLabel, { ...param }, { width: 500 })
        .subscribe((res) => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          CardLabelService.postApiAppCardLabelBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
    ...
    desgin(item) {
      window.open(AppConsts.cardLabelBaseUrl + '/Design/label?id=' + item.id);
    },
  },
});
