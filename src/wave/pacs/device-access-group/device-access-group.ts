...
import CreateoOrEditDeviceAccessGroup from './create-or-edit-device-access-group/create-or-edit-device-access-group.vue';
  ...
  methods: {
    ...
    createOrEdit(item?) {
      let param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewDeviceAccessGroup'
            : 'Unified.texts.EditDeviceAccessGroup',
      };
      this.modalHelper.create(CreateoOrEditDeviceAccessGroup, { ...param }, { size: 'medium' }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsDeviceAccessGroupService.deleteApiAppPacsDeviceAccessGroup({
        id: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      let param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditDeviceAccessGroup',
      };
      this.modalHelper.create(CreateoOrEditDeviceAccessGroup, { ...param }, { size: 'medium' }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsDeviceAccessGroupService.postApiAppPacsDeviceAccessGroupBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
