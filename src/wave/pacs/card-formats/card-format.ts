import { PacsCardFormatService } from '@/apis';
...
import CreateoOrEditCardFormatfrom from './create-or-edit-card-format/create-or-edit-card-format.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsCardFormatService.getApiAppPacsCardFormatPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewCardFormat' : 'Unified.texts.EditCardFormat',
      };
      this.modalHelper.create(CreateoOrEditCardFormatfrom, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsCardFormatService.deleteApiAppPacsCardFormat({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditCardFormat',
      };
      this.modalHelper.create(CreateoOrEditCardFormatfrom, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsCardFormatService.postApiAppPacsCardFormatBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
