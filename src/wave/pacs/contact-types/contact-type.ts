import { PacsContactTypeService } from '@/apis';
...
import CreateoOrEditContactTypefrom from './create-or-edit-contact-type/create-or-edit-contact-type.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsContactTypeService.getApiAppPacsContactTypePaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewContactType' : 'Unified.texts.EditContactType',
      };
      this.modalHelper.create(CreateoOrEditContactTypefrom, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsContactTypeService.deleteApiAppPacsContactType({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditContactType',
      };
      this.modalHelper.create(CreateoOrEditContactTypefrom, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsContactTypeService.postApiAppPacsContactTypeBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
