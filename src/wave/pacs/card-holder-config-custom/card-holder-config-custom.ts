import { PacsCardHolderConfigCustomService } from '@/apis';
...
import CreateoOrEditPacsCardHolderConfigCustom from './create-or-edit-card-holder-config-custom/create-or-edit-card-holder-config-custom.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsCardHolderConfigCustomService.getApiAppPacsCardHolderConfigCustomPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    handleActionClick(event: string, item?: any) {
      switch (event) {
        case EVENT_BTN_ENUM.CONFIGURATIONcUSTOMfIELDS:
          this.createOrEdit();
          break;
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.ConfigurationCustomFields'
      };
      this.modalHelper.create(CreateoOrEditPacsCardHolderConfigCustom, { ...param }, { size:'medium' }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsCardHolderConfigCustomService.deleteApiAppPacsCardHolderConfigCustom({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      this.modalHelper.create(CreateoOrEditPacsCardHolderConfigCustom, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
        }
      });
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsCardHolderConfigCustomService.postApiAppPacsCardHolderConfigCustomBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
