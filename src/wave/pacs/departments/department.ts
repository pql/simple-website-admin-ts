import { PacsDepartmentService } from '@/apis';
...
import CreateoOrEditDepartment from './create-or-edit-department/create-or-edit-department.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsDepartmentService.getApiAppPacsDepartmentPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewDepartment' : 'Unified.texts.EditDepartment',
      };
      this.modalHelper.create(CreateoOrEditDepartment, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsDepartmentService.deleteApiAppPacsDepartment({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditDepartment',
      };
      this.modalHelper.create(CreateoOrEditDepartment, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsDepartmentService.postApiAppPacsDepartmentBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
