import { PacsPublicHolidayService } from '@/apis';
...
import CreateoOrEditpublicHoliday from './create-or-edit-public-holiday/create-or-edit-public-holiday.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return PacsPublicHolidayService.getApiAppPacsPublicHolidayPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      let param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewPublicHoliday'
            : 'Unified.texts.EditPublicHoliday',
      };
      this.modalHelper.create(CreateoOrEditpublicHoliday, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsPublicHolidayService.deleteApiAppPacsPublicHoliday({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      let param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditPublicHoliday',
      };
      this.modalHelper.create(CreateoOrEditpublicHoliday, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsPublicHolidayService.postApiAppPacsPublicHolidayBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
