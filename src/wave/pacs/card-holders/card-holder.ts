...
import CreateoOrEditCardHolder from './create-or-edit-card-holder/create-or-edit-card-holder.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsCardHolderService.getApiAppPacsCardHolderPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewCardHolder' : 'Unified.texts.EditCardHolder',
      };
      this.modalHelper
        .create(CreateoOrEditCardHolder, { ...param }, { size: 'large' })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsCardHolderService.deleteApiAppPacsCardHolder({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditCardHolder',
      };
      this.modalHelper.create(CreateoOrEditCardHolder, { ...param }, { size: 'large' }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
        }
      });
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsCardHolderService.postApiAppPacsCardHolderBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
