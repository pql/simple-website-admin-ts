...
import CreateoOrEditCompany from './create-or-edit-company/create-or-edit-company.vue';
  ...
  methods: {
    ...
    createOrEdit(item?) {
      let param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewCompany' : 'Unified.texts.EditCompany',
      };
      this.modalHelper.create(CreateoOrEditCompany, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsCompanyService.deleteApiAppPacsCompany({
        id: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      let param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditCompany',
      };
      this.modalHelper.create(CreateoOrEditCompany, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsCompanyService.postApiAppPacsCompanyBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
