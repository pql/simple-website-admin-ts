import { PacsCardHolderStatusService } from '@/apis';
...
import CreateoOrEditCardHolderStatus from './create-or-edit-card-holder-status/create-or-edit-card-holder-status.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList() {
      ...
      return PacsCardHolderStatusService.getApiAppPacsCardHolderStatusPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewCardHolderStatus'
            : 'Unified.texts.EditCardHolderStatus',
      };
      this.modalHelper.create(CreateoOrEditCardHolderStatus, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
    ...
    deleteItem(item?) {
      ...
      PacsCardHolderStatusService.deleteApiAppPacsCardHolderStatus({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditCardHolderStatus',
      };
      this.modalHelper.create(CreateoOrEditCardHolderStatus, { ...param }, { width: 500 }).subscribe((res) => {
        if (res) {
          (this.$refs.table as any).reloadGoFirstPage();
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          PacsCardHolderStatusService.postApiAppPacsCardHolderStatusBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
