import { BuildingService } from '@/apis';
...
import CreateoOrEditBuilding from './create-or-edit-building/create-or-edit-building.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return BuildingService.getApiAppBuildingPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewBuilding' : 'Unified.texts.EditBuilding',
      };
      this.modalHelper
    ...
    deleteItem(item?) {
      ...
      BuildingService.deleteApiAppBuilding({
        input: item.id,
      })
        .finally(() => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          BuildingService.postApiAppBuildingBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
