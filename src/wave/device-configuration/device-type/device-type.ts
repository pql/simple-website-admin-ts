...
import CreateoOrEditDeviceType from './create-or-edit-device-type/create-or-edit-device-type.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return DeviceTypeService.getApiAppDeviceTypePaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewDeviceType' : 'Unified.texts.EditDeviceType',
      };
      this.modalHelper
        .create(CreateoOrEditDeviceType, { ...param }, { size: 'medium' })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      DeviceTypeService.deleteApiAppDeviceType({
        input: item.id,
      })
        .finally(() => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          DeviceTypeService.postApiAppDeviceTypeBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
