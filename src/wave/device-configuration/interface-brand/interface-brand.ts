import { InterfaceBrandService } from '@/apis';
...
import CreateoOrEditInterfaceBrand from './create-or-edit-interface-brand/create-or-edit-interface-brand.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return InterfaceBrandService.getApiAppInterfaceBrandPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewInterfaceBrand'
            : 'Unified.texts.EditInterfaceBrand',
      };
      this.modalHelper
        .create(CreateoOrEditInterfaceBrand, { ...param }, { width: 500 })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      InterfaceBrandService.deleteApiAppInterfaceBrand({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditInterfaceBrand',
      };
      this.modalHelper
        .create(CreateoOrEditInterfaceBrand, { ...param }, { width: 500 })
        .subscribe((res) => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          InterfaceBrandService.postApiAppInterfaceBrandBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
