...
import {
  InterfaceBrandService,
  FireBytes_Unified_Wave_InterfaceBrands_Dtos_InterfaceBrandEditDto,
...
export default defineComponent({
  ...
  data() {
    const databases: FireBytes_Unified_Wave_InterfaceBrands_Dtos_InterfaceBrandEditDto = {
      brandName: '',
    };
  ...
  methods: {
    ...
    getEditdata() {
      if (!this.id) return;
      this.loading = true;
      InterfaceBrandService.getApiAppInterfaceBrandForEdit({ input: this.id })
        .then((res) => {
          this.form = res?.interfaceBrand;
        })
        .catch((err) => {
    ...
    nameVerify(rule, value, callback) {
      ...
      InterfaceBrandService.postApiAppInterfaceBrandNameRepeatVerify({
        requestBody: verifyInput,
      }).then((res) => {
        if (!res) {
          callback(this.l('Unified.texts.BrandNameReuse'));
          return;
        }
        callback();
      });
    ...
    handleSubmit(closeFlag = true) {
      (this.$refs.formRef as any).validate().then(() => {
        this.loading = true;
        InterfaceBrandService.postApiAppInterfaceBrandSave({
          requestBody: { interfaceBrand: this.form },
        })
          .then(() => {
