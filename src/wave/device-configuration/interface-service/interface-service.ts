import { InterfaceServiceService } from '@/apis';
...
import CreateoOrEditInterfaceService from './create-or-edit-interface-service/create-or-edit-interface-service.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return InterfaceServiceService.getApiAppInterfaceServicePaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewInterfaceService'
            : 'Unified.texts.EditInterfaceService',
      };
      this.modalHelper
        .create(CreateoOrEditInterfaceService, { ...param }, { width: 800 })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      InterfaceServiceService.deleteApiAppInterfaceService({
        id: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditInterfaceService',
      };
      this.modalHelper
        .create(CreateoOrEditInterfaceService, { ...param }, { width: 800 })
        .subscribe((res) => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          InterfaceServiceService.postApiAppInterfaceServiceBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
