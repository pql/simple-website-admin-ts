import { DeviceCredentialProfileService } from '@/apis';
...
import CreateOrEditDeviceCredentialProfile from './create-or-edit-device-credential-profile/create-or-edit-device-credential-profile.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return DeviceCredentialProfileService.getApiAppDeviceCredentialProfilePaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _titleName:
          item?.id == null
            ? 'Unified.texts.CreateNewDeviceCredentialProfile'
            : 'Unified.texts.EditDeviceCredentialProfile',
      };
      this.modalHelper
        .create(CreateOrEditDeviceCredentialProfile, { ...param }, { width: 600 })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      DeviceCredentialProfileService.deleteApiAppDeviceCredentialProfile({
        input: item.id,
      })
        .finally(() => {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditDeviceCredentialProfile',
      };
      this.modalHelper
        .create(CreateOrEditDeviceCredentialProfile, { ...param }, { width: 600 })
        .subscribe((res) => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          DeviceCredentialProfileService.postApiAppDeviceCredentialProfileBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
