...
import CreateOrEditDeviceBrand from './create-or-edit-device-brand/create-or-edit-device-brand.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return BrandService.getApiAppBrandPaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName:
          item?.id == null ? 'Unified.texts.CreateNewBrand' : 'Unified.texts.EditBrand',
      };
      this.modalHelper
        .create(CreateOrEditDeviceBrand, { ...param }, { width: 800 })
        .subscribe((res) => {
          if (res) {
    ...
    deleteItem(item?) {
      ...
      BrandService.deleteApiAppBrand({
        input: item.id,
      })
        .finally(() => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          BrandService.postApiAppBrandBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
