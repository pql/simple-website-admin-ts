...
import CreateoOrEditDeviceZone from './create-or-edit-device-zone/create-or-edit-device-zone.vue';
...
export default defineComponent({
  ...
  methods: {
    fetchDataList(otherQuery) {
      ...
      return ZoneService.getApiAppZonePaged(queryFilter);
    },
    /**
     * 按钮回调事件
     */
    ...
    createOrEdit(item?) {
      const param = {
        ...
        _types: '',
        _titleName: item?.id == null ? 'Unified.texts.CreateNewZone' : 'Unified.texts.EditZone',
      };
      this.modalHelper
        .create(CreateoOrEditDeviceZone, { ...param }, { size: 'large' })
        .subscribe((res) => {
          if (res) {
    ...
    bulkEdit() {
      ...
      const param = {
        ...
        _types: '',
        _titleName: 'Unified.texts.EditZone',
      };
      this.modalHelper
        .create(CreateoOrEditDeviceZone, { ...param }, { size: 'large' })
        .subscribe((res) => {
    ...
    batchDelete() {
      ...
      this.confirm({
        ...
        onOk: () => {
          (this.$refs.table as any).setLoading(true);
          ZoneService.postApiAppZoneBatchDelete({
            requestBody: ids,
          })
            .finally(() => {
              (this.$refs.table as any).setLoading(false);
