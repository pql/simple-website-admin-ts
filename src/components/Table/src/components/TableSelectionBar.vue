  ...
  <a-alert type="info" showIcon :class="[prefixCls]">
    <template #message>
      <span v-if="props.count > 0">
        {{ t('Unified.texts.component:table:selectionBarTips', { count: props.count }) }}
      </span>
      <span v-else>
        {{ t('Unified.texts.component:table:selectionBarEmpty') }}
      </span>
      <a-button type="link" @click="clearSelectedRowKeys" size="small" v-show="props.count > 0">
        {{ t('Unified.texts.component:table:selectionBarClear') }}
      </a-button>
    </template>
  </a-alert>
</template>
