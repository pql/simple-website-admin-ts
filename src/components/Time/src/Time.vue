...
<script lang="ts" setup>
  ...
  function getRelativeTime(timeStamp: number) {
    ...
    let resStr = '';
    let dirStr = isBefore
      ? t('Unified.texts.component:time:before')
      : t('Unified.texts.component:time:after');

    if (diff < ONE_SECONDS) {
      resStr = t('Unified.texts.component:time:just');
      // Less than or equal to 59 seconds
    } else if (diff < ONE_MINUTES) {
      resStr = parseInt(diff / ONE_SECONDS) + t('Unified.texts.component:time:seconds') + dirStr;
      // More than 59 seconds, less than or equal to 59 minutes and 59 seconds
    } else if (diff >= ONE_MINUTES && diff < ONE_HOUR) {
      resStr = Math.floor(diff / ONE_MINUTES) + t('Unified.texts.component:time:minutes') + dirStr;
      ...
      resStr = Math.floor(diff / ONE_HOUR) + t('Unified.texts.component:time:hours') + dirStr;
      ...
      resStr = Math.floor(diff / ONE_DAY) + t('Unified.texts.component:time:days') + dirStr;
