<template>
  ...
  >
    ...
    <Icon v-if="getIcon" class="menu-menu_icon" :icon="getIcon" :size="16" />
    <div v-if="collapsedShowTitle && getIsCollapseParent" class="mt-1 collapse-title">
      <!-- {{ getI18nName }} -->
      <SimpleTooltip :text="getI18nName" :maxWidth="120"> </SimpleTooltip>
    </div>
    <template #title>
  ...
  >
    <template #title>
      ...
      <span v-show="getShowSubTitle" :class="['ml-4', `${prefixCls}-sub-title`]">
        <!-- {{ getI18nName }} -->
        <SimpleTooltip :text="getI18nName" :maxWidth="120"> </SimpleTooltip>
      </span>
...
<script lang="ts" setup>
  ...
  import { SimpleTooltip } from '@/components/EllipsisText';
  ...
  const isOverflowTip = ref<boolean>(false);
  const textElement = ref<HTMLElement | null>(null);
  const checkOverflow = () => {
    console.log('textElement', textElement.value);
    const canvas = document.createElement('canvas');
    const context = canvas.getContext('2d');
    // const width = context.measureText(text).width;
    // if (textElement.value && textElement.value > 117) {

    //   isOverflowTip.value = true;
    // } else {

    //   isOverflowTip.value = false;
    // }
  };
  onMounted(() => {
    nextTick(() => {
